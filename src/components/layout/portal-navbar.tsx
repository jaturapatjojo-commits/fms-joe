...
import { LayoutDashboard, User, LogOut, Settings, Menu, X } from "lucide-react";
...
const NAV_LINKS = [
  { href: "/portal", label: "หน้าหลัก" },
  { href: "/portal/news", label: "ข่าวสาร" },
  { href: "/portal/curriculum", label: "หลักสูตร" },
  { href: "/portal/staff", label: "บุคลากร" },
  { href: "/portal/reservations", label: "จองห้อง/ยานพาหนะ" },
  { href: "/portal/documents", label: "สารบรรณ" },
  { href: "/portal/contact", label: "ติดต่อ" },
];
...
export function PortalNavbar({
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 w-full border-b border-border/60 bg-[var(--glass-strong)] backdrop-blur-md shadow-xs",
        className
      )}
    >
      <div className="h-16 px-4 md:px-8 flex items-center justify-between">
        ...
        <button
          type="button"
          className="lg:hidden p-1.5 ml-1 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors"
    ...
    {mobileOpen && (
      <nav className="lg:hidden border-t border-border/60 bg-background/95 backdrop-blur-md px-4 py-3 space-y-1 animate-in fade-in-50 slide-in-from-top-2">
        {NAV_LINKS.map((item) => {
          const isActive =
            item.href === "/portal"
              ? pathname === "/portal"

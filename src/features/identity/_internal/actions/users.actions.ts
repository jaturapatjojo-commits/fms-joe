...
import { listUsersQuerySchema, createUserSchema, updateUserSchema, setUserActiveSchema, issuePasswordLinkSchema, requestEmailChangeSchema, exportUsersQuerySchema, importUsersInputSchema } from "../validations/users";
...
export async function exportUsersCsvAction(q: unknown): Promise<ActionResult<string>> {
  return runAction(async () => {
    const ctx = await requirePermission(P.usersRead);
    const data = exportUsersQuerySchema.parse(q, await em());
    return svc.exportUsersCsv(ctx.tenantId, data);
  });
}

export async function importUsersCsvAction(input: unknown): Promise<ActionResult<svc.ImportResult>> {
  return runAction(async () => {
    const ctx = await requirePermission(P.usersManage);
    const data = importUsersInputSchema.parse(input, await em());
    return svc.importUsersCsv({ ...actorOf(ctx), ...data });
  });
}
